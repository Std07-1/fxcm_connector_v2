...
def test_tail_guard_invalidation_on_upsert(tmp_path: Path, monkeypatch) -> None:
    ...
    assert first.tf_states["1m"].skipped_by_ttl is False

    updated_bar = dict(bars[-1])
    updated_bar["close"] = 1.5
    updated_bar["ingest_ts_ms"] = now_ms + 1
    store.upsert_1m_final("XAUUSD", [updated_bar])
    ...
    assert second.tf_states["1m"].skipped_by_ttl is False
